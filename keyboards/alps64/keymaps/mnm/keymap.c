#include QMK_KEYBOARD_H

#define CTL_X    MT(MOD_LCTL, KC_X)
#define ALT_Z    MT(MOD_LALT, KC_Z)
#define CTL_DOT  MT(MOD_RCTL, KC_DOT)
#define ALT_SLSH MT(MOD_LALT, KC_SLSH)

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  /* 0: qwerty */
  LAYOUT_all( \
    KC_GESC, KC_1,    KC_2,    KC_3,   KC_4,   KC_5,   KC_6,   KC_7,   KC_8,   KC_9,    KC_0,    KC_MINS,  KC_EQL,  KC_NUHS, KC_DEL, \
    KC_TAB,  KC_Q,    KC_W,    KC_E,   KC_R,   KC_T,   KC_Y,   KC_U,   KC_I,   KC_O,    KC_P,    KC_LBRC,  KC_RBRC, KC_BSLS, \
    KC_BSPC, KC_A,    KC_S,    KC_D,   KC_F,   KC_G,   KC_H,   KC_J,   KC_K,   KC_L,    KC_SCLN, KC_QUOT,  KC_ENT,  \
    KC_LSFT, KC_NUBS, ALT_Z,   CTL_X,  KC_C,   KC_V,   KC_B,   KC_N,   KC_M,   KC_COMM, CTL_DOT, ALT_SLSH, KC_RSFT, KC_ESC, \
    KC_LCTL, KC_LALT, KC_LGUI,                 KC_SPC,                                  KC_APP,  MO(1),    KC_RALT, KC_RCTL
  ),
  /* 1: L1 */
  LAYOUT_all(                                                           \
    KC_GRV,  KC_F1,   KC_F2,   KC_F3,   KC_F4,   KC_F5,   KC_F6,   KC_F7,   KC_F8,   KC_F9,   KC_F10,  KC_F11,  KC_F12,  KC_TRNS,  KC_INS,  \
    _______, _______, KC_UP,   _______, RESET,   _______, _______, _______, _______, _______, _______, _______, _______, _______, \
    KC_CAPS, KC_LEFT, KC_DOWN, KC_RGHT, _______, _______, _______, _______, _______, _______, _______, _______, _______, \
    _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, \
    _______, _______, _______,                   _______,                                     _______, KC_HOME, _______, KC_END
  ),
};
